...
function validateRelations(relations: RelationTypes.Relation): true | VQLError {
    if (!isObj(relations, false)) return emptyErr("The 'relations' property must be an object.");

    for (const key in relations) {
        const relation = relations[key];
        if (!isObj(relation)) return emptyErr(`Relation '${key}' must be an object.`);
        if (!("path" in relation) || !Array.isArray(relation.path) || relation.path.length !== 2) return emptyErr(`Relation '${key}' must have a 'path' property with an array of two strings.`);
        if ("search" in relation && !isObj(relation.search, false)) return emptyErr(`Relation '${key}' has an invalid 'search' property; it must be an object.`);
        if ("many" in relation && typeof relation.many !== "boolean") return emptyErr(`Relation '${key}' has an invalid 'many' property; it must be a boolean.`);
        if ("options" in relation && !isObj(relation.options, false)) return emptyErr(`Relation '${key}' has an invalid 'options' property; it must be an object.`);
        if ("select" in relation && !Array.isArray(relation.select)) return emptyErr(`Relation '${key}' has an invalid 'select' property; it must be an array.`);

        if ("relations" in relation) {
            const nestedResult = validateRelations(relation.relations as RelationTypes.Relation);
            if (nestedResult !== true) return nestedResult;
        }
    }
...
function validR(query: VQL_Query_Relation): true | VQLError {
    ...
    if (!isObj(r.relations, false)) return emptyErr("Relation query 'r' must have a 'relations' object.");

    const relationsValidation = validateRelations(r.relations);
    if (relationsValidation !== true) return relationsValidation;
...
function validD(query: VQL_Query_CRUD): true | VQLError {
    ...
    if (key === "add") {
        const op = d.add!;
        if (!isObj(op.data)) return emptyErr("'add' operation requires a 'data' object.");
        if ("id_gen" in op && typeof op.id_gen !== "boolean") return emptyErr("'add' operation 'id_gen' property must be a boolean.");
        return true;
    }
    ...
    if (key === "update" || key === "updateOne") {
        ...
        if (!isObj(op.search, false)) return emptyErr(`'${key}' operation requires a 'search' object.`);
        if (!isObj(op.updater, false)) return emptyErr(`'${key}' operation requires an 'updater' object.`);
        return true;
    }

    if (key === "updateOneOrAdd") {
        const op = d.updateOneOrAdd!;
        if (!isObj(op.search, false)) return emptyErr("'updateOneOrAdd' operation requires a 'search' object.");
        if (!isObj(op.updater, false)) return emptyErr("'updateOneOrAdd' operation requires an 'updater' object.");
        if ("add_arg" in op && !isObj(op.add_arg, false)) return emptyErr("'updateOneOrAdd' operation 'add_arg' property must be an object.");
        if ("id_gen" in op && typeof op.id_gen !== "boolean") return emptyErr("'updateOneOrAdd' operation 'id_gen' property must be a boolean.");
        return true;
    }
