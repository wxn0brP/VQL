...
function parseInstancePath(path: string): string[] {
    if (!path) return [];
...
function finalizeNode(node: any): void {
    if (node.__errors) {
        node.__errors = node.__errors.map((e: InternalError) => e.message);
    }
...
export function buildAjvErrorTree(errors: ErrorObject[]): Record<string, any> {
    ...
    for (const error of errors) {
        ...
        if (isComposite) {
            ...
            const hasComposite = currentNode.__errors.some((e: InternalError) =>
                ["anyOf", "oneOf", "allOf"].includes(e.keyword)
            );
            if (!hasComposite) {
