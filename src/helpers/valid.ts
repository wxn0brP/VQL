...
export function validateRaw(query: VQL_Query): true | VQLError {
	...
	return emptyErr(
		"Invalid VQL query structure. Must contain 'r' or 'db' and 'd'.",
	);
}
...
function validateRelations(relations: RelationTypes.Relation): true | VQLError {
	...
	for (const key in relations) {
		...
		if (
			!("path" in relation) ||
			!Array.isArray(relation.path) ||
			relation.path.length !== 2
		)
			return emptyErr(
				`Relation '${key}' must have a 'path' property with an array of two strings.`,
			);
		if ("search" in relation && !isObj(relation.search, false))
			return emptyErr(
				`Relation '${key}' has an invalid 'search' property; it must be an object.`,
			);
		if ("many" in relation && typeof relation.many !== "boolean")
			return emptyErr(
				`Relation '${key}' has an invalid 'many' property; it must be a boolean.`,
			);
		if ("options" in relation && !isObj(relation.options, false))
			return emptyErr(
				`Relation '${key}' has an invalid 'options' property; it must be an object.`,
			);
		if ("select" in relation && !Array.isArray(relation.select))
			return emptyErr(
				`Relation '${key}' has an invalid 'select' property; it must be an array.`,
			);

		if ("relations" in relation) {
			const nestedResult = validateRelations(
				relation.relations as RelationTypes.Relation,
			);
...
function validR(query: VQL_Query_Relation): true | VQLError {
	...
	if (!("path" in r) || !Array.isArray(r.path) || r.path.length !== 2)
		return emptyErr(
			"Relation query 'r' must have a 'path' property with an array of two strings.",
		);
...
function validD(query: VQL_Query_CRUD): true | VQLError {
	...
	if (typeof value.collection !== "string" || value.collection.trim() === "")
		return emptyErr(
			`CRUD operation '${key}' must specify a non-empty 'collection' string.`,
		);

	if (
		key === "issetCollection" ||
		key === "ensureCollection" ||
	...
	if (key === "find") {
		...
		if ("select" in op && !isObj(op.select, false) && !Array.isArray(op.select))
			return emptyErr(
				"'find' operation 'select' property must be an object or an array.",
			);
		...
		if ("searchOpts" in op && !isObj(op.searchOpts, false))
			return emptyErr(
				"'find' operation 'searchOpts' property must be an object.",
			);
		return true;
	}
	...
	if (key === "findOne" || key === "f") {
		...
		if ("searchOpts" in op && !isObj(op.searchOpts, false))
			return emptyErr(
				`'${key}' operation 'searchOpts' property must be an object.`,
			);
		return true;
	}
	...
	if (key === "updateOneOrAdd") {
		...
		if (!isObj(op.updater, false))
			return emptyErr(
				"'updateOneOrAdd' operation requires an 'updater' object.",
			);
		if ("add_arg" in op && !isObj(op.add_arg, false))
			return emptyErr(
				"'updateOneOrAdd' operation 'add_arg' property must be an object.",
			);
		if ("id_gen" in op && typeof op.id_gen !== "boolean")
			return emptyErr(
