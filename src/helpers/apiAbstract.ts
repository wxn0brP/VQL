...
export type ResolverFn<TArg = any, TReturn = any> = (
	query: TArg,
) => Promise<TReturn>;
...
export interface ValtheraResolver {
	...
	updateOneOrAdd?: ResolverFn<
		VQueryT.UpdateOneOrAdd<Data, false>,
		VQueryT.UpdateOneOrAddResult<Object>
	>;
	toggleOne?: ResolverFn<
		VQueryT.ToggleOne<Data, false>,
		VQueryT.ToggleOneResult<Object>
	>;
...
export function createValtheraAdapter(
	resolver: ValtheraResolver,
	extendedFind: boolean = false,
): ValtheraCompatible {
	const safe = <T>(fn?: T): T => {
...
export class AdapterBuilder {
	...
	constructor(
		private catchCb: (e: any, op: string, arg: any) => void = e => {
			console.log(e);
		},
	) {}
	...
	removeCollection(
		collection: string,
		fn: ValtheraResolver["removeCollection"],
	) {
	...
	getAdapter(extendedFind: boolean = true) {
		const resolve = async (op: string, arg: any) => {
			const col: string =
				typeof arg === "object" && "collection" in arg ? arg.collection : arg;

			const handler =
				this.handlers.get(`${op}:${col}`) ||
				this.handlers.get(`${op}:*`) ||
				null;
		...
		const adapter = createValtheraAdapter(
			{
				...
				ensureCollection: async col => {
					if (!this.collections.has(col)) this.collections.add(col);
					return true;
				},
			},
			extendedFind,
