...
export function createValtheraAdapter(resolver, extendedFind = false) {
    ...
    adapter.c = (collection) => new Collection(adapter, collection);
    for (const name of list) {
        // @ts-expect-error
        adapter[name] = (query) => safe(resolver[name])(query);
    }
    if (extendedFind) {
        ...
        adapter.findOne = async (query) => {
            const data = await safe(resolver.findOne)(query);
            if (typeof data !== "object")
                return data;
            return updateFindObject(data, query.findOpts || {});
        };
    }
    return adapter;
}
export class AdapterBuilder {
    ...
    getAdapter(extendedFind = true) {
        const resolve = async (op, arg) => {
            const col = typeof arg === "object" && "collection" in arg ? arg.collection : arg;
            const handler = this.handlers.get(`${op}:${col}`) ||
        ...
        for (const name of list.slice(4)) {
            // @ts-expect-error
            adapter[name] = (arg) => resolve(name, arg);
        }
        return adapter;
    }
}
