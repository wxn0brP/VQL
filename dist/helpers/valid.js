...
function validD(query, cfg) {
    ...
    if (key === "updateOneOrAdd") {
        ...
        if ("select" in op && !isObj(op.select, false) && !Array.isArray(op.select))
            return emptyErr("'updateOneOrAdd' operation 'select' property must be an object or an array.");
        return true;
    }
    if (key === "toggleOne") {
        ...
        if ("select" in op && !isObj(op.select, false) && !Array.isArray(op.select))
            return emptyErr("'toggleOne' operation 'select' property must be an object or an array.");
        return true;
    }
    const n = key;
    return emptyErr(`Unknown or invalid CRUD operation: '${key}'`);
}
export function validateVql(query, cfg = new VQLConfig()) {
    if ("r" in query && isObj(query.r))
        return validR(query);
    if ("d" in query && isObj(query.d))
        return validD(query, cfg);
