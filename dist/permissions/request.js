...
export async function checkRequestPermission(config, permValidFn, user, query) {
    ...
    const checkPermissionRecursively = async (entityId, originalPath, requiredPerm, fallbackLevels = []) => {
        ...
        if (!config.strictACL &&
            ...
            return checkPermissionRecursively(nextFallbackEntityId, nextFallbackLevels, requiredPerm, nextFallbackLevels);
        }
...
export async function filterObjectByPermissions(config, permValidFn, user, db, collection, obj) {
    if (!obj || typeof obj !== "object")
        return obj;
    const allPaths = extractAllPaths(obj);
    const checkFieldPermission = async (path) => {
        const fullPath = [
            db,
            collection,
            ...path,
...
export async function filterObjectsByPermissions(config, permValidFn, user, db, collection, objects) {
    if (!Array.isArray(objects))
        return objects;
    const result = [];
    for (const obj of objects) {
        result.push(await filterObjectByPermissions(config, permValidFn, user, db, collection, obj));
    }
    return result;
}
