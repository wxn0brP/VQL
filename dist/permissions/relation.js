...
export async function checkRelationPermission(config, permValidFn, user, query) {
    ...
    const checkPermissionRecursively = async (entityId, originalPath, fallbackLevels = []) => {
        ...
        if (!config.strictACL &&
            ...
            return checkPermissionRecursively(nextFallbackEntityId, nextFallbackLevels, nextFallbackLevels);
        }
    ...
    if (relations) {
        for (const relationKey in relations) {
            ...
            if (!r.path || !Array.isArray(r.path))
                continue;
            const hasPermission = await checkRelationPermission(config, permValidFn, user, {
                r: {
                    path: r.path,
                    search: {},
